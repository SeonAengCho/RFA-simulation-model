...
def DataLoad():
    ...
    Y_test = torch.Tensor(np.load("data/output_Dmg_test.npy"))
...
# test trained model
def TestModel(model, dataset, th, diceloss):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(len(X_test_foreseen))):
            ...
            total_loss_foreseen.append(diceloss(outputs, targets).cpu())
            ...
            total_loss_unforeseen.append(diceloss(outputs, targets).cpu())
            
            image_unforeseen.append(np.array(outputs.detach().cpu()))
        
        
        dice_foreseen = np.mean(total_loss_foreseen)
        dice_unforeseen = np.mean(total_loss_unforeseen)
        print("Dice score (foreseen data) : {} % \n".format(1-dice_foreseen))
        print("Dice score (unforeseen data) : {} % \n".format(1-dice_unforeseen))
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('model/UNETR_Dmg.pth', map_location=device))
    
    # test
    dataset = DataLoad()
    image_foreseen, image_unforeseen = TestModel(model, dataset, thresholding,diceloss)
    ImagePlot(5, image_unforeseen, dataset[3], 1, 18)
