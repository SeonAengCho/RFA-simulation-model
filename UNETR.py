...
class Last_Deconv3DBlock(nn.Module):
    def __init__(self,in_planes, out_planes, kernel_size=3):
        super().__init__()
        self.block = nn.Sequential(
            nn.ConvTranspose3d(in_planes, in_planes, kernel_size=5, stride=1, padding=0),
            nn.ConvTranspose3d(in_planes, out_planes, kernel_size=5, stride=1, padding=0),
        )
    def forward(self, x):
        return self.block(x)
...
class UNETR(nn.Module):
    def __init__(self, img_shape=(40, 40, 40), input_dim=2, output_dim=18, embed_dim=1024, patch_size=4, num_heads=16, dropout=0.1):
        super().__init__()
        ...
        self.decoder12_upsampler = \
            SingleConv3DBlock(embed_dim, 512, 1)
    ...
    def forward(self, x):
        ...
        output = F.interpolate(output, [41, 41, 41])
